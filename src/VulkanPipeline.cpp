...
void VulkanPipeline::init(VulkanDevice* device, VulkanSwapChain* swapChain, Triangle* triangle)
{
    this->device = device;
...
void VulkanPipeline::drawFrame()
{
    ...
    this->renderPass.recordCommandBuffer(commandBuffer, *this->triangle, imageIndex);
    ...
    VkResult result = vkQueueSubmit(this->device->getGraphicsQueue(), 1, &submitInfo, this->inFlightFence);
...
void VulkanPipeline::createPipeline()
{
    ...
    VkPipelineVertexInputStateCreateInfo vertexInputInfo = createVertexInputInfo(bindingDescription, attributeDescriptions);
    ...
    VkPipelineColorBlendStateCreateInfo colorBlending = createColorBlending(&colorBlendAttachment);
    ...
    VkResult result = vkCreateGraphicsPipelines(*this->device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &this->graphicsPipeline);
    if (result != VK_SUCCESS) {
...
VkPipelineVertexInputStateCreateInfo VulkanPipeline::createVertexInputInfo(const VkVertexInputBindingDescription& bindingDescription,
...
VkPipelineColorBlendStateCreateInfo VulkanPipeline::createColorBlending(VkPipelineColorBlendAttachmentState* colorBlendAttachment)
{
