...
uint32_t VulkanBuffer::findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties)
{
    ...
    vkGetPhysicalDeviceMemoryProperties(this->device->getPhysicalDevice(), &memProperties);
...
void VulkanBuffer::copyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size)
{
    ...
    vkQueueSubmit(this->device->getGraphicsQueue(), 1, &submitInfo, VK_NULL_HANDLE);
    vkQueueWaitIdle(this->device->getGraphicsQueue());

    vkFreeCommandBuffers(*this->device, this->renderPass->getCommandPool(), 1, &commandBuffer);
}
